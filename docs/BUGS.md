...
| B-011 | P3 | OPEN | In Source mode, document clicks outside the editor can still trigger a restart from the current cursor sentence | Start playback in Source mode, click ribbon/status bar area, observe potential restart from current cursor sentence | `plugin/src/view/sourceModeHooks.ts` | v1.2 uses a global click hook plus editor cursor offset to avoid DOM-based text mapping |
