...
| T-056 | Replace explicit synth UI with on-demand playback synthesis and start-index-aware suffix generation | DONE | agent | T-055 | Removed synth ribbon/command, Play now auto-prepares cache on demand, and Source-mode restart synthesizes only the requested sentence suffix |
| T-057 | Isolate temp staging per synthesis run and auto-repair unusable cached WAVs | DONE | agent | T-056 | Temp output now uses unique per-run staging folders to avoid concurrent-run file collisions, and playback prep re-synthesizes missing/invalid WAV headers automatically |
| T-058 | Fix false “outdated sentence hashes” notice on first-play of uncached notes | DONE | agent | T-057 | Cache validation now treats missing manifest as stale only when cache files exist; brand-new notes report missing audio path instead of outdated-hash warning |
| T-059 | Fix Windows cache file discovery for sentence WAV parsing | DONE | agent | T-058 | Cache listing now normalizes backslashes before filename extraction so existing `sentence-xxxx.wav` files are detected reliably on Windows paths |
| T-060 | Sync in-flight synthesis sentence state into runtime playback array | DONE | agent | T-059 | On-demand suffix synthesis now updates `this.sentences` (not only local split copy) for generating/error/ready+audioPath, fixing intermittent `unknown path` start failures |
| T-061 | Add user confirmation prompt to retry re-synthesis after playback/synthesis errors | DONE | agent | T-060 | Playback start failures and partial re-synthesis failures now prompt the user to choose whether to re-synthesize from the affected sentence onward |
| T-062 | Fix first-play ENOENT copy mismatch by aligning server output naming and hardening source WAV resolution | DONE | agent | T-061 | Server now writes one-based `sentence-xxxx.wav` names matching vault cache naming; plugin now validates/fallback-resolves synthesized source WAV path before copy and marks clear per-sentence errors when missing |
