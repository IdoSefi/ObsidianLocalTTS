...
| T-062 | Harden first-play sentence copy flow against missing server path and sentence-id mismatches | DONE | agent | T-061 | Added response `sentenceId` validation, destination-folder pre-copy guard, source existence check + base64 fallback write, and explicit diagnostics when copy input is missing |
