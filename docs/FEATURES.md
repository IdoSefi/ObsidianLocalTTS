...
| F-027 | Backend-aware synthesis settings | COMPLETE | Plugin settings persist active backend (`kokoro`/`piper`) plus separate voice settings (`kokoroVoice`, fixed `piperVoice`) and shared speed/server URL | Keeps Kokoro as default, preserves Kokoro voice when switching back from Piper, and migrates legacy `voice` data to `kokoroVoice` |
