...
| F-022 | Session playback-speed control | COMPLETE | Status bar includes compact expandable playback-rate control (`0.75x`–`1.50x`) that changes client-side `HTMLAudioElement.playbackRate` | Applies immediately to current audio and carries forward to newly created per-sentence audio elements without re-synthesis |
