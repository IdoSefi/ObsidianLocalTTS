...
| F-020 | Hash-based cache invalidation | COMPLETE | Cached playback validates full-note + per-sentence text hashes and only plays an unchanged sentence prefix | Shows stale-from-sentence notice and blocks playback into changed sentences until re-synthesis |
