...
| F-025 | Normalized text-first plugin sentence resolution | COMPLETE | Clicked rendered sentence now maps to plugin sentence by normalized text match first, then safe index fallback | Reduces wrong jumps from index drift while keeping logic simple and predictable |
| F-026 | Render-time sentence-id tagging | COMPLETE | Reading view post-processing wraps rendered sentence fragments with `data-kokoro-sentence-id`, and click handling resolves sentence id via nearest tagged ancestor | Replaces click-time DOM offset reconstruction with constant-time delegated lookup |
