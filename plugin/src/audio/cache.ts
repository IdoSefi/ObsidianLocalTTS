...
export class VaultAudioCache {
  ...
  getNoteSynthesisFolder(notePath: string, backend: PluginSettings["backend"]): string {
  ...
  async listExistingSentenceAudio(
    notePath: string,
    backend: PluginSettings["backend"],
  ): Promise<ExistingSentenceAudioResult> {
    const noteFolderPath = this.getNoteSynthesisFolder(notePath, backend);
  ...
  async clearNoteSynthesis(notePath: string, backend: PluginSettings["backend"]): Promise<void> {
  ...
  async writeManifest(
    notePath: string,
    backend: PluginSettings["backend"],
    manifest: NoteSynthesisManifest,
  ): Promise<void> {
  ...
  async readManifest(notePath: string, backend: PluginSettings["backend"]): Promise<NoteSynthesisManifest | null> {
    const manifestPath = normalizePath(`${this.getNoteSynthesisFolder(notePath, backend)}/${MANIFEST_FILE}`);
  ...
  private getSafeNoteFolderName(notePath: string, backend: PluginSettings["backend"]): string {
    ...
    return `${sanitizedBase}-${backend}-${hash}`;
  }
